...
var itemResponses = ref<ItemResponse[]>([])
var selectedDataItem = ref<ItemResponse>()
...
onMounted(async()=>{
    selectedDataItem== ref<ItemResponse>()
...
watchEffect(()=>{
    ...
    if(props.UiElement.defaultAccessPath != null){
        ...
        if(dts.length == 1){
            basecontent[dts[0]] = itemResponses.value.find((item:ItemResponse)=>{
...
const OnValueChanged=(e:ItemResponse)=>{
    var xx = itemResponses.value.find((item)=>{
        return item == e
    })
    LaunchOnClickAction(xx as ItemResponse)
}
const LaunchOnClickAction=(data : ItemResponse)=>{
