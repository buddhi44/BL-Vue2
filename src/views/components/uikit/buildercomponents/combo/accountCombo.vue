...
import type { AccountResponse } from '@/core/domain/Entities/Base/accountResponse';


const props = defineProps(["UiElement","isGrid"])

var accountResponses = ref<AccountResponse[]>([])
var selectedDataItem = ref<AccountResponse>()
...
onMounted(async()=>{
    selectedDataItem== ref<AccountResponse>()
...
watchEffect(()=>{
    ...
    if(props.UiElement.defaultAccessPath != null){
        ...
        if(dts.length == 1){
            basecontent[dts[0]] = accountResponses.value.find((item:AccountResponse)=>{
...
const OnValueChanged=(e:AccountResponse)=>{
    var xx = accountResponses.value.find((item)=>{
        return item == e
    })
    LaunchOnClickAction(xx as AccountResponse)
}
const LaunchOnClickAction=(data : AccountResponse)=>{
...
const fetchComboData=async(searchText='')=> {
    ...
    accountResponses.value=content
